...
class DenseNet:
  def __init__(self, inputTensor, growth_rate, depth,
...
def densenet_feature_extractor(inputTensor):
  ...
  return DenseNet(inputTensor, **vars(args)).output

inputTensor = tf.placeholder(tf.float32, shape=[None,32,32,3])
outputTensor = densenet_feature_extractor(inputTensor)
